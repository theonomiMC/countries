...
const Country = ({ country }) => {
    return (
        <div className="bg-gray-100 dark:bg-[#2b3945] flex items-center justify-center">
            <Link href={`/${country?.alpha2Code.toLowerCase() || country?.alpha3Code}?country=${country.name}`} passHref>
                <div className="w-80 dark:bg-[#2b3945] bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl hover:scale-105 duration-500 transform transition cursor-pointer">
                    <Image src={country?.flag} alt={country?.name} width={350} height={200} />
